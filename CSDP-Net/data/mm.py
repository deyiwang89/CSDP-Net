...
class MovingMNIST_set(data.Dataset):
    def __init__(self, root, is_train, n_frames_input, n_frames_output, num_objects,
                 transform=None):
        '''
        param num_objects: a list of number of possible objects.
        '''
        super(MovingMNIST_set, self).__init__()
    ...
    def get_random_trajectory(self, seq_length):
        ''' Generate a random sequence of a MNIST digit '''
        canvas_size = self.image_size_ - self.digit_size_
        x = 0.8#0.185#random.random()
        y = 0.7#0.44#random.random()
        theta =  0.9* 2 * np.pi#0.773 * 2 * np.pi#random.random() * 2 * np.pi
        v_y = np.sin(theta)
        v_x = np.cos(theta)
    ...
    def generate_moving_mnist(self, num_digits=2):
        ...
        for n in range(num_digits):
            # Trajectory
            start_y, start_x = self.get_random_trajectory(self.n_frames_total)
            ind = 155#random.randint(0, self.mnist.shape[0] - 1)
            digit_image = self.mnist[ind]
            for i in range(self.n_frames_total):
                top = start_y[i]
                left = start_x[i]
